<script lang="ts">
  ...
  function getPriorityDot(task: Task) {
    if (task.isMeeting) return "bg-pink-600";
  ...
  function getStatusBadge(status: string) {
    switch (status) {
      ...
      case "In progress":
        return {
          color: "bg-blue-50 text-pink-600 border-pink-600",
  ...
  <div class="flex items-center justify-between mb-5">
    <div class="flex items-center gap-4">
      ...
      <div class="flex items-center gap-2">
        ...
        <span class="text-gray-400 text-sm font-medium">{tasks.length}</span>
      </div>
    ...
    {#each tasks as task}
      ...
      >
        <div class="flex items-center gap-3.5">
          <!-- Priority Dot -->
          <div
            class={cn("w-2 h-2 rounded-full shrink-0", getPriorityDot(task))}
          ></div>
          <span
