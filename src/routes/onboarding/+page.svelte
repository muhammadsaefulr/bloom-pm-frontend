<script lang="ts">
  ...
  onMount(async () => {
    const token = localStorage.getItem("bloom_join_token");
    if (token && $isAuthenticated) {
      isLoading = true; // Block reactive redirects immediately
      inviteCode = token;
  ...
  $: if (
    $isAuthenticated &&
    ($authStore?.user?.teams?.length ?? 0) > 0 &&
    !isLoading
  ) {
    goto("/dashboard");
  }
  ...
  async function handleJoinTeam() {
    if (!inviteCode.trim()) {
      error = "Please enter an invitation code or token";
      return;
    }
    ...
    try {
      // First try to join assuming it's a generic invite link token
      try {
        await apiClient.post("/teams/member/join-link", { token: inviteCode });
      } catch (linkErr: any) {
      ...
      // Refresh user details to get the new team assignments
      const user = await getCurrentUserApi();
      ...
      console.error("Join failure:", e);
      error =
        e.response?.data?.message || "Invalid or expired invitation token.";
    } finally {
      isLoading = false;
    }
  }
...
>
  <div class="w-full max-w-md relative">
    {#if isLoading}
      <div
        transition:fade
        class="absolute inset-0 flex flex-col items-center justify-center bg-background/80 backdrop-blur-sm z-50 rounded-2xl min-h-[400px]"
      >
        <Loader2 class="w-12 h-12 text-primary animate-spin mb-4" />
        <p class="text-lg font-medium">Joining workspace...</p>
      </div>
    {/if}
      ...
      >
        <div class="space-y-4 text-center">
          <div class="mx-auto flex items-center justify-center mb-8">
            <img width="220" src="/BloomLogo.png" alt="Bloom Logo" />
          </div>
        ...
        <div class="space-y-6">
          ...
          >
            ...
            <div class="flex-1">
              <h3 class="text-xl font-medium mb-1">Create a new workspace</h3>
              <p
